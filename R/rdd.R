...
lm_q7_wide1 = lm_robust(recidivism ~ bac1, data = Q7_narrow)
coef(lm_q7_narrow1)
...
bac = vector(mode="numeric",length=214558)
BAC1 = data.frame(hansen_dwi$male,hansen_dwi$white,hansen_dwi$acc,hansen_dwi$recidivism,hansen_dwi$bac1,hansen_dwi$bac2,bac)



rdplotdensity(rddensity(hansen_dwi$bac1), X = hansen_dwi$bac1)
rdplotdensity()
rddensity(x=hansen_dwi$bac1)


ggplot(data = hansen_dwi) +
  geom_histogram(aes(x = bac1), color = 'black', binwidth = 0.001, boundary = 0.001) +
  geom_vline(xintercept = 0.08, size = 1) +
  labs(title = 'BAC Histogram', y = 'Frequency', x = 'bac1') + 
...
ggplot(data=hansen_dwi, aes=(x=bac1)) + 
  geom_histogram(binwidth=0.01,fill="#69b3a2", color="#e9ecef", alpha=0.9)+
  theme_bw() + 
  labs(x="", y="")

ggplot(data=hansen_dwi)+
  geom_histogram(binwidth=0.001, mapping = aes(bac1))

ggplot(aes(x, y1, colour = factor(D)), data = BAC) +
